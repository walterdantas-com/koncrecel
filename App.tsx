...
// Aqui você pode colar os links do Drive para as logos
const LOGO_FULL_URL = 'klogo-full.png'; 
const LOGO_ICON_URL = 'ícone-logo.png';
...
const App: React.FC = () => {
  ...
  const handleSendRequest = (method: 'whatsapp' | 'email') => {
    ...
    if (method === 'whatsapp') {
      ...
      const subject = encodeURIComponent('Orçamento - Catálogo Koncrecel');
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-[2.5rem] w-full max-w-sm shadow-2xl">
              ...
              <div className="space-y-4">
                <input className="w-full p-4 bg-gray-50 rounded-xl border-gray-100 border text-xs outline-none focus:border-blue-500" placeholder="URL Planilha Produtos" value={sheetUrl} onChange={e => setSheetUrl(e.target.value)} />
                <input className="w-full p-4 bg-gray-50 rounded-xl border-gray-100 border text-xs outline-none focus:border-blue-500" placeholder="URL Planilha Segurança" value={securityUrl} onChange={e => setSecurityUrl(e.target.value)} />
              </div>
              <div className="flex flex-col gap-2 mt-8">
                <button onClick={() => { localStorage.setItem('koncrecel_sheet_url', sheetUrl); localStorage.setItem('koncrecel_security_url', securityUrl); setShowConfig(false); fetchData(); }} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-xs tracking-widest shadow-lg">Salvar</button>
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-md">
          ...
          >
            ...
            <div className="flex items-center space-x-2 bg-white/10 px-4 py-2 rounded-full">
              <span className="text-[10px] font-black uppercase">Enviar Orçamento</span>
      ...
      {showSendPicker && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[110] flex items-center justify-center p-6">
          <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm text-center shadow-2xl">
            <h3 className="text-xl font-black uppercase mb-2 text-gray-900 leading-tight">Como deseja enviar sua solicitação?</h3>
            <p className="text-xs text-gray-400 mb-8 font-medium">Escolha o canal de sua preferência</p>
            <div className="grid grid-cols-1 gap-3">
              <button 
                onClick={() => handleSendRequest('whatsapp')}
                className="flex items-center justify-center space-x-3 bg-green-500 text-white p-5 rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-transform shadow-lg shadow-green-500/20"
              >
              ...
              <button 
                ...
                className="flex items-center justify-center space-x-3 bg-blue-600 text-white p-5 rounded-2xl font-black uppercase text-xs tracking-widest active:scale-95 transition-transform shadow-lg shadow-blue-500/20"
              >
